...
function M.draw_board_background()
  ...
  lutro.graphics.setColor(60, 40, 20, 255)
  lutro.graphics.rectangle("fill", bx - pad, by - pad,
    tw + pad * 2, th + pad * 2)

  lutro.graphics.setColor(100, 70, 30, 255)
  lutro.graphics.rectangle("fill", bx - pad + 4, by - pad + 4,
    tw + pad * 2 - 8, th + pad * 2 - 8)

  lutro.graphics.setColor(40, 25, 10, 255)
  lutro.graphics.rectangle("line", bx - pad + 2, by - pad + 2,
...
function M.draw_layer_cutouts(tiles)
  ...
  for i, t in ipairs(tiles) do
    if not t.removed and above[i] then
      ...
      lutro.graphics.setColor(15, 20, 50, 200)
      lutro.graphics.rectangle("fill", x + 4, y + 4, conf.TILE_W - 8, conf.TILE_H - 8)
    end
  end
end
  ...
  for l = 1, conf.MAX_LAYERS do
    for i, t in ipairs(tiles) do
      if not t.removed and t.layer == l then
        ...
        if ok then
          lutro.graphics.setColor(255, 255, 255, 255)
        else
          lutro.graphics.setColor(80, 80, 80, 180)
        end

        lutro.graphics.draw(M.tileset, M.quads[t.type], x, y)

        if i == highlight_idx then
          lutro.graphics.setColor(255, 255, 0, 120)
          lutro.graphics.rectangle("fill", x, y, conf.TILE_W, conf.TILE_H)
        end

        if i == selected_idx then
          lutro.graphics.setColor(0, 255, 0, 160)
          lutro.graphics.rectangle("fill", x, y, conf.TILE_W, conf.TILE_H)
        end
      end
    end
...
function M.draw_text(text, x, y, r, g, b)
  lutro.graphics.setColor(r or 255, g or 255, b or 255, 255)
  local space_width = M.font and M.font:getWidth("I") or 5
  for word, spaces in text:gmatch("([^ ]*)( *)") do
    if word ~= "" then
      lutro.graphics.print(word, x, y)
...
function M.draw_status(msg)
  if msg then
    ...
    lutro.graphics.setColor(255, 220, 0, 255)
    lutro.graphics.rectangle("fill", M.vw / 2 - w / 2 - 6, M.vh / 2 - 16, w + 12, 24)
...
function M.draw_game_over(win)
  ...
  local w = M.text_width(msg)
  lutro.graphics.setColor(0, 0, 0, 220)
  lutro.graphics.rectangle("fill", 0, M.vh / 2 - 40, M.vw, 80)
